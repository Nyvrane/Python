...
class AnaPencere(QWidget):
    def __init__(self):
        ...
        # Başlık etiketini oluşturalım ve dikey düzene ekleyelim
        label = QLabel("Hisse Sembolü:")
        vbox.addWidget(label)

        # Arama kutusunu oluşturalım ve dikey düzene ekleyelim
        self.textbox = QLineEdit()
        vbox.addWidget(self.textbox)

        # Arama butonunu oluşturalım ve dikey düzene ekleyelim
        ...
        # Butona tıklandığında anafonksiyon() fonksiyonunu çağıralım
        self.button.clicked.connect(self.anafonksiyon)

    # Ana foksiyonumuzu oluşturalım
    def anafonksiyon(self):
        # Butonu devre dışı bırakalım
        ...
        def birincikriter(donenvarliklar, kisavadeyukum, finansalgelir, finansalgider, brutkar):
            ...
            if int(donenvarliklar) - int(kisavadeyukum) >= 0:
                self.output.append("1.Kriter - İYİ: Şirket borçlarını ödeyebilecek durumda")
                puan += 20
                pass
            else:
                ...
                if int(finansalgelir) >= int(finansalgider) and int(brutkar) >= 0:
                    self.output.append("1.Kriter - ORTA: Şirket borçlarını ödeyebilecek durumda değil fakat kar ediyor")
                    puan += 10
                else:
                    self.output.append("1.Kriter - KÖTÜ: Şirket borçlarını ödeyecek durumda değil ve karsız. Dikkat!")
        ...
        def ikincikriter(nakit, kisavadefinansalborc, kisavadeyatirim):
            ...
            if int(nakit) >= int(kisavadefinansalborc):
                self.output.append("2.Kriter - İYİ: Dönen varlıkların kalitesi iyi.")
                puan += 20
            elif int(nakit) + int(kisavadeyatirim) >= int(kisavadefinansalborc):
                self.output.append("2.Kriter - ORTA: Şirketin borçlarını ödemek için "
                                   "nakiti yetmiyor fakat yatırımları eklenince kısa vade borçlarını karşılıyor")
                puan += 10
            else:
                self.output.append("2.Kriter - KÖTÜ: Şirket kısa vade borçlarını karşılayacak durumda değil. "
                                   "Alacakların şüpheli alacak olup olmadığını kontrol etmek gerekebilir. Dikkat!")
        ...
        def ucuncukriter(toplamvarliklar, ozkaynaklar):
            ...
            if 1 >= int(ozkaynaklar) / (int(toplamvarliklar)) >= 0.7:
                self.output.append("3.Kriter - İYİ: Mali Yapısı Güçlü")
                puan += 20
            elif 0.7 > int(ozkaynaklar) / (int(toplamvarliklar)) >= 0.4:
                self.output.append("3.Kriter - ORTA: Mali Yapısı Dengeli")
                puan += 10
            elif 0.4 > int(ozkaynaklar) / (int(toplamvarliklar)) >= 0:
                self.output.append("3.Kriter - KÖTÜ: Mali Yapısı Güçsüz")
            elif int(ozkaynaklar) < 0:
                self.output.append("3.Kriter - ÇOK KÖTÜ: Şirket Özkaynakları ekside, iflas aşamasında olabilir.")
                puan -= 5
